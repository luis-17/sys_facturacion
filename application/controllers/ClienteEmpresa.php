...
class ClienteEmpresa extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->helper(['security', 'imagen_helper', 'otros_helper', 'fechas_helper']);
        $this->load->model(['model_cliente_empresa', 'model_categoria_cliente']);
        //cache
    ...
    public function editar()
    {
        ...
        // VALIDACIONES
        /* VALIDAR SI EL RUC YA EXISTE */
        $fCliente = $this->model_cliente_empresa->m_validar_cliente_empresa_num_documento($allInputs['ruc'], true,
            $allInputs['id']);
        if ($fCliente) {
            $arrData['message'] = 'El RUC ingresado, ya existe.';
            $arrData['flag'] = 0;
